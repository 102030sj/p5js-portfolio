...
function drawCharacter(cx, cy) {
  push();
  translate(cx - 300, cy - 120); // 기준점 이동
  ...
  fill(clothR, clothG, clothB); // top color
  ...
  if (isNaked) {
    fill(255, 210, 185);
    noStroke();
    ellipse(300 - 45, 270, 6, 6); // 왼쪽
    ellipse(300 + 45, 270, 6, 6); // 오른쪽
    
    fill(180, 100, 100);
    ellipse(300, 370, 2, 10);
  }
  ...
  fill(hairR, hairG, hairB); //hair detail
...
function keyPressed() {
  if (keyCode === LEFT_ARROW) charX -= speed;
  if (keyCode === RIGHT_ARROW) charX += speed;
  if (keyCode === UP_ARROW) charY -= speed;
  if (keyCode === DOWN_ARROW) charY += speed;

  charY = constrain(charY, minY, height); // 위쪽 제한
